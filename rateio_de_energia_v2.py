...
tarifas = {
    ...
    "band_ate_150": st.sidebar.number_input("Bandeira até 150 kWh", value=0.054400, format="%.6f"),
    "band_acima_150": st.sidebar.number_input("Bandeira acima 150 kWh", value=0.057660, format="%.6f"),
}
cosip = st.sidebar.number_input("COSIP (R$)", value=17.01, format="%.2f")

# Escolha do método de rateio
st.sidebar.header("📊 Método de Rateio")
metodo_rateio = st.sidebar.radio(
    "Escolha o método:",
...
def calcular_valor(consumo):
    ...
    band = c1 * tarifas["band_ate_150"] + c2 * tarifas["band_acima_150"]
...
if st.button("Calcular"):
    ...
    }, index=[f"Quitinete {i+1}" for i in range(n)])
